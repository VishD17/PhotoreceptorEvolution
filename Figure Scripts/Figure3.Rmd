...
##PRs (No Integration)
pdf("/Users/vishruthdinesh/Downloads/Figures/Fig_3/3a_noint.pdf", w=6, h=6, useDingbats = FALSE)
DimPlot(ph, group.by = "species")
...
p = DimPlot(OrthoObject2, group.by = "seurat_clusters")
p + ggtitle("Photoreceptor OrthoTypes")
```
...
types_plot <- c(types_plot, which(max_type == "oLM-cone"))
types_plot <- c(types_plot, which(max_type == "oS-cone"))
types_plot <- rownames(type_table)[types_plot]

OT_order <- c( "oS-cone","oLM-cone", "oRod")
seuratcluster_species_order <- c("human_1","marmoset_2", "mouse_rod", "squirrel_4","cow_0","lizard_11","chicken_1","human_3", "marmoset_0", "mouse_l/m-cone", "squirrel_0", "cow_3","lizard_1", "chicken_4","human_2", "marmoset_5", "mouse_s-cone",  "squirrel_3","cow_5", "lizard_8", "chicken_7")
...
ggplot(data = df, aes(x=OT, y=Species_Type, fill=mapping)) +
  ...
  guides(fill=guide_legend(title="Mapping %")) + 
    ...
    scale_fill_gradientn(colors = colors, na.value = "gray70") + scale_x_discrete(limits = levels(df$OT)) +coord_flip()
dev.off()
```
