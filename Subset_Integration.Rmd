...
#mouse[["RNA"]]$counts <- as.sparse(mouse[["RNA"]]$counts)
#mouse[["RNA"]]$data <- as.sparse(mouse[["RNA"]]$data)
...
marmoset_subset = subset(int_marmoset, cells = WhichCells(int_marmoset, downsample = 140, seed = 12345))
...
gene_lists <- list(cow_genes, squirrel_genes)
...
ph <- merge(ortho_cow, y = c(ortho_squirrel), add.cell.ids = c("cow","squirrel"), project = "merge")
table(ph$species)
```


```{r}
#Integrating using ClusterSeruat function 
cow$species <- "cow"
...
seurat_colors <- c("0" = "brown1", "1" = "cornflowerblue", "2" = "limegreen", "3" = "limegreen")
DimPlot(OrthoObject2, group.by = "celltype")
DimPlot(OrthoObject2, group.by = "species")
DimPlot(OrthoObject2, reduction = "umap")
DimPlot(OrthoObject2, split.by = "species", group.by = "celltype")
...
```{r}
#LM Cone
lmcone_markers_all <- FindMarkers(OrthoObject2, ident.1 = "1", ident.2 = NULL, only.pos = TRUE)
head(lmcone_markers_all, n = 20)
DimPlot(OrthoObject2, split.by = "species", group.by = "celltype")
```
