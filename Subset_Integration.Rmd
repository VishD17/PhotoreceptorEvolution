```{r}
url <- "https://cran.r-project.org/src/contrib/Archive/Matrix/Matrix_1.6-1.tar.gz"
install.packages(url, repos = NULL, Source = TRUE)
sessionIn
```
...
mouse$seurat_clusters <- sub("0", "l/m-cone", mouse$seurat_clusters)
mouse$seurat_clusters <- sub("1", "s-cone", mouse$seurat_clusters)
mouse$seurat_clusters <- sub("2", "l/m-cone", mouse$seurat_clusters)
mouse$seurat_clusters <- sub("3", "rod", mouse$seurat_clusters)
mouse$celltype <- mouse$seurat_clusters
...
int_marmoset <- marmoset_int[marmoset_de_genes,]
...
Idents(cow) = "celltype"
cow$celltype <- sub("Rod", "rod", cow$celltype)
cow$celltype <- sub("ML_cone", "l/m-cone", cow$celltype)
...
human$celltype <- sub("mlCones", "l/m-cone", human$celltype)
...
#SaveH5Seurat(human, "/Users/vishruthdinesh/Downloads/Indv_Objects/Human_PR_Final.h5Seurat")
```
...
mouse_subset = subset(mouse, cells = WhichCells(mouse, downsample = 82, seed = 12345))
marmoset_subset = subset(int_marmoset, cells = WhichCells(marmoset_int, downsample = 140, seed = 12345))
chicken_subset = subset(chicken, cells = WhichCells(chicken, downsample = 50, seed = 12345))
...
squirrel_subset = subset(squirrel, cells = WhichCells(squirrel, downsample = 250, seed = 12345))
...
gene_lists <- list(cow_genes, squirrel_genes, marmoset_genes, lizard_genes, mouse_genes, chicken_genes)
...
#Rod
rod_markers_marmoset <- FindMarkers(marmoset_int, ident.1 = "rod", ident.2 = NULL, only.pos = TRUE)
head(rod_markers_marmoset, n = 20)
marmoset_rod_list <- rownames(rod_markers_marmoset)

#S-cone
scone_markers_marmoset <- FindMarkers(marmoset_int, ident.1 = "s-cone", ident.2 = NULL, only.pos = TRUE)
head(scone_markers_marmoset, n = 20)
marmoset_scone_list <- rownames(scone_markers_marmoset)

#LM Cone
lmcone_markers_marmoset <- FindMarkers(marmoset_int, ident.1 = "l/m-cone", ident.2 = NULL, only.pos = TRUE)
...
chicken_double_list <- chicken_double_list
...
#Rod
rod_markers_mouse <- FindMarkers(mouse, ident.1 = "rod", ident.2 = NULL, only.pos = TRUE)
